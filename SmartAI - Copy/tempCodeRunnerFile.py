...
def classify_intent(text: str) -> str:
    ...
    - ask_starter: When user specifically asks about starters
    - ask_dessert: When user specifically asks about desserts
...
def build_assistant(vs_id: str):
    asst = client.beta.assistants.create(
        ...
        3. When asked about recommendations, consider:
           ...
           - For kids: in breakfast mention 'Poha','Upma','Sheera'
           - For kids: in lunch mention 'Dal Fry','Paneer Butter Masala'
           - For kids: in dinner mention 'Kadai Paneer','Dal Makhani'
...
def get_ai_response(thread, assistant, user_input, context=None):
    # Prepare the system message with enhanced menu knowledge
    system_message = {
        "role": "system",
...
def process_starters(menu_data: dict, meal_period: str, collected_orders: list, context: list):
    ...
    speak("What would you like to order from our starters?")
    ...
    while True:
        ...
        if is_negative_response(cmd):
            speak("No problem, we'll skip starters.")
            return collected_orders, False, context
        ...
        if structured_orders:
            for order_item in structured_orders:
                ...
                if dish_data:
                    ...
                    speak(f"Sorry, we don't have {dish_name_raw} in our starters.")
            return collected_orders, False, context
        else:
            speak("I didn't quite get that. Could you tell me the starter name and quantity?")
...
def process_desserts(menu_data: dict, meal_period: str, collected_orders: list, context: list):
    ...
    speak("What would you like to order from our desserts?")
    ...
    while True:
        ...
        if is_negative_response(cmd):
            speak("No problem, we'll skip desserts.")
            return collected_orders, False, context
        ...
        if structured_orders:
            for order_item in structured_orders:
                ...
                if dish_data:
                    ...
                    speak(f"Sorry, we don't have {dish_name_raw} in our desserts.")
            return collected_orders, False, context
        else:
            speak("I didn't quite get that. Could you tell me the dessert name and quantity?")
...
def confirm_and_place_order(collected_orders: list, context: list):
    ...
    while True:
        ...
        context.append(f"User: {confirm_input}")
        ...
        if any(word in confirm_cmd for word in ["yes", "please", "confirm"]):
            ...
            try:
                ...
                speak(response_text)
                context.append(f"Assistant: {response_text}")
                return [], True, context
            except Exception as e:
...
# === Dynamic Order Flow Functions ===
def handle_starter_first_flow(menu_data: dict, meal_period: str, collected_orders: list, context: list):
    # Process starters first
    collected_orders, exit_flag, context = process_starters(menu_data, meal_period, collected_orders, context)
    if exit_flag:
        return collected_orders, True, context

    # Then ask about main courses
    speak("Would you like to discover any main course dishes?")
    while True:
        user_input = listen()
        if not user_input:
    ...
    # Then ask about desserts
    speak("Would you like to finish your meal with desserts?")
    while True:
        user_input = listen()
        if not user_input:
        ...
        if is_positive_response(cmd):
            ...
            speak(response)
            context.append(f"Assistant: {response}")
            speak("Would you like any desserts to finish your meal?")

    return collected_orders, False, context

def handle_dessert_first_flow(menu_data: dict, meal_period: str, collected_orders: list, context: list):
    # Process desserts first
    collected_orders, exit_flag, context = process_desserts(menu_data, meal_period, collected_orders, context)
    if exit_flag:
        return collected_orders, True, context

    # Then ask about main courses
    speak("Would you like to try something from the main course?")
    while True:
        user_input = listen()
        if not user_input:
            speak("I didn't catch that. Would you like to see our main courses?")
    ...
    # Then ask about starters if no main courses were ordered
    if not any(item for item in collected_orders if any(key in item['name'].lower() for key in MAIN_COURSE_CATEGORY_KEYWORDS)):
...
def handle_default_flow(menu_data: dict, meal_period: str, collected_orders: list, context: list):
    # Start with a simple prompt
    #speak("What would you like to order?")
    
    user_input = listen()
    if not user_input:
        speak("I didn't hear your order. Could you please repeat it?")
        return collected_orders, False, context
    ...
    # First try to process as direct order
    intent = classify_intent(user_input)
    if intent == INTENT_ORDER or any(word in user_input.lower() for word in ["order", "want", "would like", "get", "have"]):
        structured_orders = extract_structured_order(user_input)
        if structured_orders:
            for order_item in structured_orders:
                dish_name_raw = order_item.get("name", "").strip()
                ...
                if dish_data:
                    ...
                    speak(f"Sorry, we don't have {dish_name_raw} on our menu.")
            
            # After processing direct order, suggest complementary items
            if collected_orders:
                # Check if ordered item is a dessert
                ordered_item = collected_orders[-1]['name'].lower()
                is_dessert = any(keyword in ordered_item for keyword in ["dessert", "sweet", "ice cream", "gulab jamun", "rasgulla"])
                
                if not is_dessert:
                    # Suggest starters
                    speak("Would you like any starters while your food is being prepared?")
                    starter_input = listen()
                    if starter_input and is_positive_response(starter_input):
                        collected_orders, exit_flag, context = process_starters(menu_data, meal_period, collected_orders, context)
                        if exit_flag:
                            return collected_orders, True, context
                    ...
                    # Suggest desserts
                    speak("Would you like any desserts to finish your meal?")
                    dessert_input = listen()
                    if dessert_input and is_positive_response(dessert_input):
                        collected_orders, exit_flag, context = process_desserts(menu_data, meal_period, collected_orders, context)
                        if exit_flag:
                            return collected_orders, True, context
                
                # Proceed to order confirmation
                collected_orders, order_placed, context = confirm_and_place_order(collected_orders, context)
                return collected_orders, order_placed, context
    
    # If not a direct order, proceed with menu browsing
    speak("Would you like to see our menu options?")
    while True:
        user_input = listen()
        if not user_input:
            speak("I didn't catch that. Would you like to see our menu options?")
            continue
        ...
        if is_positive_response(user_input):
            # Show main courses first
            collected_orders, exit_flag, context = process_main_courses(menu_data, meal_period, collected_orders, context)
            if exit_flag:
                return collected_orders, True, context
            
            # Then suggest starters
            speak("Would you like any starters while your main course is being prepared?")
            while True:
                user_input = listen()
                if not user_input:
                    speak("I didn't catch that. Would you like any starters?")
...
if __name__ == "__main__":
    ...
    try:
        ...
        conversation_context = []  # Track conversation history for context
        ...
        while True:
            ...
            if intent == INTENT_STARTER:
                collected_orders, exit_flag, conversation_context = handle_starter_first_flow(menu_data, meal_period, collected_orders, conversation_context)
                if exit_flag:
                    break

                collected_orders, order_placed, conversation_context = confirm_and_place_order(collected_orders, conversation_context)
                if order_placed:
                    continue

            elif intent == INTENT_DESSERT:
                collected_orders, exit_flag, conversation_context = handle_dessert_first_flow(menu_data, meal_period, collected_orders, conversation_context)
                if exit_flag:
                    break
            ...
            elif intent == INTENT_MENU or intent == INTENT_ORDER:
                collected_orders, exit_flag, conversation_context = handle_default_flow(menu_data, meal_period, collected_orders, conversation_context)
                if exit_flag:
                    break
